...
- How are tau leptons reconstructed in CMS?
...
- Learn the basic elements of tau lepton reconstruction

::::::::

## Overview of muon reconstruction

Muons are the *M* in CMS (Compact Muon Solenoid).  This is in part because they are reconstructed using all the CMS sub-detectors.  As it nicely summarized [here](https://cms.cern/detector/detecting-muons):
...
While uons *can* be reconstructed using only the muon system, the best measurements come from combining information from the muon detectors and the tracking detectors. Muons that leave clear signals in both of these detectors are called "global muons". Muons have many features in common with electrons, but their own unique identification algorithms. We will use the same NanoAOD file as in the previous episode to look at the Muon NanoAOD branches.
...
The CMS Muon object group has several identification algorithms and the pass/fail decisions for several working points are stored in NanoAOD.
...
Just as we discussed for electrons, muons that are produced right away during a collision are usually separated from
the other objects in the event, or *isolated*. For instance, an isolated muon might be produced in the decay of a W boson.
In contrast, a non-isolated muon might come from hadrons decaying inside a jet.

Muon isolation is calculated from a combination of factors: energy from charged hadrons, energy from
neutral hadrons, and energy from photons, all in a cone around
the muon. Two cone sizes are used: either radii of 0.3 or 0.4, measured in angular units. Many algorithms also feature a *correction factor* that subtracts average energy expected
from extra simultaneous collisions to this cone. This energy sum is then divided by the muon's transverse momentum to form "relative" isolation. The whole calculation looks like this: 
